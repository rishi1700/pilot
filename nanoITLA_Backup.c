...
static int c_handle_register(uint8_t reg,uint8_t isw,uint16_t data,uint8_t *xe_out,uint16_t *d_out){
  ...
  * ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------
     * LUT debug window (temporary registers 0x70–0x78)
     *
     * These are NOT spec registers, just a convenient way to read back
     * the current LUT row that nano_apply_channel_from_lut() loaded
     * into g_active_channel_50GHz / globals in nano_globals.c.
     *
     * Scaling (host will divide back):
     *   - Voltages:  V * 100   -> centi-volts
     *   - Currents:  mA * 10   -> deci-mA
     *   - Temp:      °C * 10   -> deci-°C
     *   - PDs:       value * 10 (placeholder)
