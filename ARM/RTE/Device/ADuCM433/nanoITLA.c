...
static int c_handle_register(uint8_t reg,uint8_t isw,uint16_t data,uint8_t *xe_out,uint16_t *d_out){
  ...
  switch(reg){
    ...
    case 0x2A: if(isw){ st_ALMT=data; } else d=st_ALMT; break;
    case 0x30:
      if(isw){
        if(data>9999){ xe=1; g_last_error=LERR_RVE; }
        else { st_CHANNEL=data; set_busy(1); pending_cnt=3; }
      } else d=st_CHANNEL; break;
